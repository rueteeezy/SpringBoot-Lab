...
![System diagram with Accounts functionality](src/site/assets/extended-system-diagram.png)
...
You will start with the `36-mvc` project. Navigate to that project now in your IDE.
...
* Once it is running navigate to `http://localhost:8080` in your browser. You should see a page like this:
    ...
    ![Home page](src/site/assets/mvc-home-page.png)

    The "List accounts" link does not work yet.

* If you are interested, the original HTML is in `src/main/resources/templates/index.html`.
...
* If you see the accounts in JSON format all is well.
    ...
    * Internet Explorer: Who uses this?!?

**TODO-06**: Stop the application

* We will be making several changes and running tests and don't need Devtools to keep restarting our application when we are not using it. _Stop the application now._

**TODO-07**: Test the controller

* A unit test for our controller has already been written for you. It uses a `StubAccountManager` managing a single test `Account`.
* Open `AccountControllerTests` and run its tests now. It should pass.
* Strictly speaking we should have tested the controller before running the application, but as the application was already running we exercised it first. This is not normal development practice - _always run tests first!_
...
* You should already have `AccountControllerTests` open. Scroll down to `handleDetailsRequest()` and add code to invoke the new method on the controller and verify the results as instructed by the TODOs.

**TODO-10**: Test the new controller method

* Return to `AccountControllerTests` and remove the `@Disabled` annotation on the `handleDetailsRequest()` method.
