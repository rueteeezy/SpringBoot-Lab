...
In reasonably complex projects you may want to keep JPA coupled code independent of POJO domain objects, and use either _Proxies_ or _Adapters_ to decouple them.

**TODO-03**: Implement the `AccountRepository` to be a Spring Data JPA Repository.

* Make the `AccountRepository` to be a Spring Data JPA `Repository<T, ID>` interface.
...
However ☝️, since this is a Spring Boot enabled test and auto-configuration is enabled, you will find that it is _not necessary_ to do any of these. This allows you to focus on configuring just the custom values needed to override default configurations.
...
You may be wondering _why are there two application properties?_ 🤔
...
You did not implement JPA for `RewardRepository`. Why not?

If you look carefully at the `JdbcRewardRepository.confirmReward()` method, you will notice the naming of the method is more of a business vocabulary than repository. It does not map to Spring Data conventions.

Could you port `RewardRepository` to JPA? How might you do this?
