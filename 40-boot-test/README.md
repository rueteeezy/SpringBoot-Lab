...
Note that the `pom.xml` (and `build.gradle`) of the parent project already has `spring-boot-starter-test`.
...
Observe, in the console log, that Spring Boot Testing framework started an embedded Tomcat server on a random port.
...
**TODO-09**: Create `AccountManager` mock bean using `@MockitoBean` annotation
...
The type `MockBean` has been deprecated since Spring Boot version `3.4.0` and marked for removal in 4.0.0. Use `MockitoBean` instead.
...
##### Experiment With `@MockitoBean` vs `@Mock` (Optional)

**TODO-13 (Optional)**: Experiment with `@MockitoBean` vs `@Mock`

1. Change `@MockitoBean` to `@Mock` for the `AccountManager` dependency.
...
Add another `MockMvc` test for getting all accounts. The test should verify the following:
